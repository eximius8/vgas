...
async def test_job_marked_finished(
    ...
    db_session,
    monkeypatch, partner_a_correct_response, partner_b_correct_response):
...
async def test_job_marked_finished_partial_success(
    sample_database_data,
    db_session,
    monkeypatch, partner_a_correct_response):
    async def fetch_partner(*args, **kwargs):
    ...
    assert job.stats.get("partnerA").get('transformed') == len(partner_a_correct_response)
