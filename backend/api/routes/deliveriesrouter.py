...
def create_job(
    *,
    ...
    Create new job.
    """
    created, job = jobscrud.get_or_create_job(session=session, job_create=job_in)
    if created:
...
@router.get("/jobs/{job_id}", response_model=JobStatusSerializer)
...
@router.get("/jobs/{job_id}/results", response_model=DeliveryListSerializer)
def get_job_results(
    *,
    session: SessionDep,
    job_id: UUID,
...
def get_all_results(
    *,
    ...
    Get all results.
    """
    deliveries = deliveriescrud.get_deliveries(session=session, filters=filters)
    return {
        "items": deliveries.get("items", []),
        "total": deliveries.get("total", 0),
        "limit": filters.limit,
