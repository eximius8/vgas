    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if not isinstance(data, list):
                raise ValueError(
                    f"Expected list from {partner_url}, got {type(data)}: {data}"
                )
            return data
            ...
            logger.error(
                f"HTTP status error fetching {partner_url}: {e.response.status_code} - {e}"
            )
            raise
...
async def fetch_partners() -> List[List[dict] | Exception]:
    ...
    partner_a_results, partner_b_results = await asyncio.gather(
        fetch_partner_a, fetch_partner_b, return_exceptions=True
    )
